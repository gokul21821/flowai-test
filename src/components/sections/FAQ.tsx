...
// Custom scrollbar classes - only visible on tablets and mobiles
...
export default function FAQ() {
  ...
  // Animation variants helper for slide directions
  const getSlideVariants = () => {
    const isEven = activeCategoryIndex % 2 === 0;
    const xOffset = 30;
    
    return {
      initial: { 
        opacity: 0, 
        x: isEven ? -xOffset : xOffset 
      },
      animate: { 
        opacity: 1, 
        x: 0 
  ...
  return (
    <section className="w-full bg-white py-[100px] px-5 md:px-20 flex flex-col items-center gap-16">
      ...
      <div className="w-full max-w-[1280px] flex flex-col gap-8 items-center">
            ...
            {categories.map((category, index) => {
              ...
              return (
                <button
                  ...
                  className={`
                    px-3 md:px-4 lg:px-3 py-4 rounded-[48px] text-sm font-medium transition-all duration-200 whitespace-nowrap
                    ${
                      isActive
                        ? "bg-white text-[#130F0C] shadow-sm"
                        : "text-[#130F0C]/70 hover:bg-white/50 hover:text-[#130F0C]"
                    }
                  `}
                >
                  <AnimatePresence mode="wait">
                    <motion.span
                      ...
                      initial={{ x: categoryIndex % 2 === 0 ? -30 : 30 }}
                      animate={{ x: 0 }}
                      exit={{ x: categoryIndex % 2 === 0 ? 30 : -30 }}
        ...
        {/* Accordion Container */}
        <div className="w-full bg-[rgba(245,245,245,0.4)] rounded-[30px] p-2 md:p-4">
